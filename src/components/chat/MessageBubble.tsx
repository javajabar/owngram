      ...
      <div 
        ...
        className={cn(
            message.attachments?.some((a: any) => a.type === 'image')
                ? "max-w-xs px-0 py-0 shadow-sm relative text-sm inline-flex items-end gap-1.5 transition-all duration-200 ease-out"
        ...
        {voiceAttachment ? (
            ...
            <div className="space-y-2">
                {message.attachments.filter((a: any) => a.type === 'image').map((attachment: any, idx: number) => (
                    <div
                        key={idx}
                        className="relative block rounded-lg overflow-hidden max-w-xs cursor-pointer"
                        onClick={(e) => {
                            e.stopPropagation()
                            if (onImageClick) {
